    ...
    @if($collectors->count() > 0)
        ...
        <div class="flex space-x-4 overflow-x-auto pb-4 scrollbar-hide" id="collector-carousel-track">
            @foreach($collectors as $index => $collector)
                ...
                {{-- Collector Card Item --}}
                <article class="collector-carousel-item flex-shrink-0 w-72 group relative bg-gray-800/50 rounded-xl p-4 border border-gray-700/50 hover:border-gray-600 hover:bg-gray-800/70 transition-all duration-300"
                    ...
                    @if($showBadgeItem)
                        <div class="flex justify-between items-start mb-3">
                            ...
                            {{-- Rank Badge --}}
                            @if($rank)
                                <div class="flex items-center justify-center w-8 h-8 text-sm font-bold text-white rounded-full bg-gradient-to-r from-yellow-400 to-orange-500 ring-2 ring-gray-800">
                                    #{{ $rank }}
                                </div>
                            @endif
                        </div>
                    ...
                    <div class="text-center mb-4">
                        <a href="{{ route('collector.home', ['id' => $collector->id]) }}"
                           class="block relative mx-auto w-20 h-20 rounded-full overflow-hidden bg-gradient-to-br from-gray-700 to-gray-800 group-hover:ring-2 group-hover:ring-emerald-400 transition-all duration-300">
                            ...
                            <img src="{{ $imageUrl }}" 
                                 alt="{{ $collector->first_name }} {{ $collector->last_name }}"
                                 class="w-full h-full object-cover transition-transform duration-300 group-hover:scale-110"
                                 loading="lazy">

                            {{-- Hover overlay --}}
                            <div class="absolute inset-0 bg-emerald-400/20 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
                                <svg class="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M13.5 6H5.25A2.25 2.25 0 003 8.25v7.5A2.25 2.25 0 005.25 18h7.5A2.25 2.25 0 0015 15.75v-7.5A2.25 2.25 0 0013.5 6z" />
                        ...
                        {{-- Nome Collector --}}
                        <h3 class="mt-3 font-semibold text-white text-sm group-hover:text-emerald-300 transition-colors duration-200 line-clamp-2">
                            <a href="{{ route('collector.home', ['id' => $collector->id]) }}">
