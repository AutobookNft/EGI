...
<a href="{{ route('home.collections.show', $collection->id) }}"
   class="collection-card group relative block aspect-[3/4] w-full overflow-hidden rounded-lg shadow-md hover:shadow-xl transition-shadow duration-300 ease-in-out bg-gray-800">
    ...
    @else
        {{-- Placeholder --}}
        <div class="absolute inset-0 flex h-full w-full items-center justify-center bg-gradient-to-br from-gray-700 via-gray-800 to-gray-900">
            <svg class="h-16 w-16 text-gray-500 opacity-50" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true">
    ...
    <div class="absolute bottom-0 left-0 right-0 p-4 text-white flex flex-col">
        {{-- Nome Collezione --}}
        <h3 class="text-lg font-semibold truncate mb-1" style="text-shadow: 1px 1px 3px rgba(0,0,0,0.8);" title="{{ $collection->collection_name }}">
            {{ $collection->collection_name }}
        </h3>
        {{-- Riga Creator --}}
        <div class="flex items-center text-xs mb-2 opacity-90" style="text-shadow: 1px 1px 2px rgba(0,0,0,0.7);">
            @if ($collection->creator)
                ...
                @if ($collection->creator->usertype === 'verified')
                     <svg class="ml-1 w-3 h-3 text-blue-400 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 1 0 0-16 8 8 0 0 0 0 16Zm3.857-9.809a.75.75 0 0 0-1.214-.882l-3.483 4.79-1.88-1.88a.75.75 0 1 0-1.06 1.061l2.5 2.5a.75.75 0 0 0 1.06 0l4-5.5Z" clip-rule="evenodd" /></svg>
                 @endif
            @else
