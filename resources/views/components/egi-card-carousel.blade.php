...
<div class="egi-carousel-container space-y-3">
    @if($egis->count() > 0)
    @foreach($egis as $egi)
    <x-egi-card-list :egi="$egi" :context="$context" :portfolioOwner="$portfolioOwner"
        :showPurchasePrice="$showPurchasePrice" :showOwnershipBadge="$showOwnershipBadge" :showBadge="$showBadge" />
    @endforeach
    @else
    {{-- Empty State --}}
...
<div class="egi-carousel-container">
    ...
    <div class="flex pb-4 space-x-4 overflow-x-auto scrollbar-hide" id="egi-carousel-track">
        ...
        {{-- EGI Card Item --}}
        <article
            class="egi-carousel-item {{ $isHyper ? 'egi-card--hiper' : '' }} flex-shrink-0 w-72 group relative bg-gray-800/50 rounded-xl p-4 border border-gray-700/50 hover:border-gray-600 hover:bg-gray-800/70 transition-all duration-300"
            ...
            <div class="relative mb-3">
                <a href="{{ route('egis.show', $egi->id) }}"
                    ...
                    @if($egi->image_url)
                    <img src="{{ $egi->image_url }}" alt="{{ $egi->title }}"
                        class="object-cover w-full h-40 rounded-lg shadow-md" loading="lazy">
                    @else
                    <div
                        class="flex items-center justify-center w-full h-40 rounded-lg bg-gradient-to-br from-gray-700 to-gray-900">
            ...
            <div class="space-y-2">
                {{-- Titolo --}}
                <h3
                    class="text-sm font-semibold text-white transition-colors duration-200 group-hover:text-purple-300 line-clamp-2">
                    <a href="{{ route('egis.show', $egi->id) }}">
                        {{ $egi->title }}
                    </a>
                </h3>
                ...
                <div class="flex items-center space-x-2">
                    <div class="flex-shrink-0 w-6 h-6 overflow-hidden bg-gray-700 rounded-full">
                        ...
                        <img src="{{ $egi->user->profile_image }}" alt="{{ $egi->user->name }}"
                            class="object-cover w-full h-full">
                        @else
                        <div
                            class="flex items-center justify-center w-full h-full bg-gradient-to-br from-purple-500 to-blue-500">
    ...
    <div class="absolute top-0 right-0 w-8 h-full pointer-events-none bg-gradient-to-l from-gray-900 to-transparent">
    </div>
    @else
    {{-- Empty State --}}
