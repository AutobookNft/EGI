{{--
Currency Price Display Component - EUR Only System with Mobile Abbreviation

Mostra un prezzo in EUR con supporto opzionale per conversione ALGO.
Su dispositivi mobile utilizza automaticamente la notazione abbreviata (K, M, B) per risparmiare spazio.
...
@param bool $mobileAbbreviated - Usa notazione abbreviata su mobile (default: true)
...
'reservation' => null,
'size' => 'normal',
'mobileAbbreviated' => true // Nuovo parametro per abilitare notazione abbreviata su mobile
])
...
// 🎯 RESPONSIVE FORMATTING: Prepariamo entrambi i formati
$standardFormat = '€ ' . number_format($safePrice, 2, ',', '.');
$abbreviatedFormat = formatPriceAbbreviated($safePrice);
...
<div class="currency-price-container{{ $egi ? ' current-price' : '' }}"{{ $egi ? ' data-egi-id=' . $egi->id : '' }}>
    <span {{ $attributes->merge(['class' => "currency-display {$class}"]) }} data-price-display>
        ...
        @if($mobileAbbreviated)
            <span class="hidden md:inline">{{ $standardFormat }}</span>
            <span class="md:hidden">{{ $abbreviatedFormat }}</span>
        @else
            {{ $standardFormat }}
        @endif
