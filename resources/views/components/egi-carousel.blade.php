...
class="relative w-full max-w-4xl mx-auto overflow-hidden border-2 shadow-2xl rounded-xl border-white/10 backdrop-blur-lg bg-black/30"
        ...
        <div class="flex-shrink-0 w-full"
             ...
             <div class="relative aspect-[4/3] bg-black"> {{-- aspect-[4/3] o altro --}}
                ...
                <img src="{{ $imageUrl }}"
                     ...
                     class="absolute inset-0 object-contain w-full h-full" loading="lazy">

                {{-- Overlay con informazioni (Opzionale, in basso) --}}
                <div class="absolute bottom-0 left-0 right-0 p-4 text-white bg-gradient-to-t from-black/70 via-black/40 to-transparent">
                    ...
                    <p class="text-xs text-gray-300 truncate">
                        {{ __('From collection:') }}
                        <a href="{{ route('home.collections.show', $egi->collection->id) }}" class="font-medium hover:underline">
...
@if($validEgis->count() > 1)
    ...
    <button @click="prevSlide(); stopAutoplay(); startAutoplay();"
            class="absolute z-20 p-2 text-white transition transform -translate-y-1/2 rounded-full top-1/2 left-2 md:left-4 bg-black/40 hover:bg-black/60 focus:outline-none focus:ring-2 focus:ring-white"
    ...
    <button @click="nextSlide(); stopAutoplay(); startAutoplay();"
            class="absolute z-20 p-2 text-white transition transform -translate-y-1/2 rounded-full top-1/2 right-2 md:right-4 bg-black/40 hover:bg-black/60 focus:outline-none focus:ring-2 focus:ring-white"
    ...
    <div class="absolute z-30 flex space-x-2 transform -translate-x-1/2 bottom-4 left-1/2">
        @foreach ($validEgis as $index => $egi)
            <button @click="activeSlide = {{ $index + 1 }}; stopAutoplay(); startAutoplay();"
                    ...
                    class="w-2 h-2 transition duration-150 ease-in-out rounded-full"
