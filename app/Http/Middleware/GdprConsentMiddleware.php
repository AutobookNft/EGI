...
/**
 * Enhanced Authentication Controller for Jetstream Integration
 * Handles post-login GDPR consent checking
 */
...
/**
 * @Oracode Controller: Enhanced Authentication with GDPR Integration
 * 🎯 Purpose: Extends Jetstream authentication with GDPR consent checking
 * 🛡️ Privacy: Ensures consent compliance during login flow
 * 🧱 Core Logic: Post-login consent verification and redirection
 */
class AuthenticatedSessionController extends BaseController
{
    /**
     * Handle successful authentication with GDPR checks
     *
     * @param Request $request
     * @return mixed
     * @privacy-safe Verifies user consent post-authentication
     */
    public function store(Request $request)
    {
        ...
        // If authentication successful, check GDPR compliance
        if ($response->isSuccessful() && auth()->check()) {
            ...
            try {
                ...
                $needsConsentReview = $this->userNeedsConsentReview($user, $consentService);

                if ($needsConsentReview) {
                    // Redirect to GDPR consent page instead of dashboard
                    session()->flash('login_success_pending_consent', true);
                    return redirect()->route('gdpr.consent')
                        ->with('info', __('gdpr.post_login_consent_required'));
                }
                ...
                app(ErrorManagerInterface::class)->handle('POST_LOGIN_GDPR_CHECK_ERROR', [
                    'user_id' => $user->id,
                    'error' => $e->getMessage()
                ], $e);

                // Don't block login for GDPR errors, just log them
            }
        }
     ...
     * @param ConsentService $consentService
     * @return bool
     * @privacy-safe Checks user's consent status
     */
    protected function userNeedsConsentReview($user, ConsentService $consentService): bool
    {
        ...
        $userConsentData = $consentService->getUserConsentStatus($user);
        $currentPolicyVersion = config('gdpr.current_policy_version', '1.0');
