...
class AuditLogService
{
    ...
    protected array $sensitiveFields = [
        'password', 'token', 'secret', 'private_key', 'credit_card',
        'ssn', 'passport', 'driver_license', 'bank_account'
    ];
    ...
    public function logGdprAction(
        ...
        try {
            ...
            $gdprLog = GdprAuditLog::create([
                ...
                'user_agent' => $requestMetadata['user_agent'],
                'timestamp' => now(),
                'retention_until' => now()->addDays($this->retentionDays), // 7 years minimum
                'compliance_note' => "GDPR Article 30 - Record of processing activities"
            ]);
        ...
        try {
            ...
            return response()->streamDownload(function () use ($activities, $format) {
                if ($format === 'json') {
                    ...
                    // CSV headers
                    fputcsv($output, [
                        'Timestamp', 'Action', 'Category', 'Privacy Level',
                        'IP Address', 'Session ID', 'Context'
                    ]);
