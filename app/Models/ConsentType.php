...
class ConsentType extends Model
{
    ...
    {
        return $this->hasMany(UserConsent::class, 'consent_type_slug', 'slug');
    }

    /**
     * Get consent versions for this type.
     *
     * @return HasMany
     */
    public function consentVersions(): HasMany
    {
        return $this->hasMany(ConsentVersion::class, 'consent_type_slug', 'slug');
    }
    ...
    public function getRetentionDays(): ?int
    {
        ...
        // Parse common retention period formats
        if (preg_match('/(\d+)\s*years?/i', $this->retention_period, $matches)) {
            return intval($matches[1]) * 365;
        }

        if (preg_match('/(\d+)\s*months?/i', $this->retention_period, $matches)) {
            return intval($matches[1]) * 30;
        }

        if (preg_match('/(\d+)\s*days?/i', $this->retention_period, $matches)) {
