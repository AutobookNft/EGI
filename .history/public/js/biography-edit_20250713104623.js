...
function initializeTabs() {
    ...
    // Se nessun tab è attivo, attiva di default il primo (basic)
    if (![...tabButtons].some(btn => btn.classList.contains('active'))) {
        const firstButton = tabButtons[0];
        if (firstButton) firstButton.classList.add('active');
    }
    if (tabContents.every(tab => tab && tab.classList.contains('hidden'))) {
        const basicTab = document.getElementById('basic-tab');
        if (basicTab) basicTab.classList.remove('hidden');
    }
    ...
    tabButtons.forEach(button => {
        button.addEventListener('click', function() {
            ...
            // Rimuovi active da tutti i buttons
            tabButtons.forEach(btn => btn.classList.remove('active'));

            // Nascondi tutti i tab-content
            tabContents.forEach(content => content && content.classList.add('hidden'));

            // Attiva il button cliccato
            this.classList.add('active');

            // Mostra il tab corrispondente
            const showTab = document.getElementById(targetTab + '-tab');
