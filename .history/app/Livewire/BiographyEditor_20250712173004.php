...
class BiographyEditor extends Component
{
    ...
    protected $listeners = [
        ...
        'setFeaturedImageData' => 'setFeaturedImageData',
        'addGalleryImageData' => 'addGalleryImageData',
        'removeFeaturedImageData' => 'removeFeaturedImageData',
        'removeGalleryImageData' => 'removeGalleryImageData',
    ];
    ...
    public function loadBiography($biographyId)
    {
        try {
            ...
            if ($biography->media) {
                $this->featuredImageData = $biography->media['featured_image'] ?? [];
                $this->galleryImagesData = $biography->media['gallery'] ?? [];
            }
    ...
    public function save()
    {
        ...
        try {
            ...
            // Handle media uploads (now handled via JavaScript)
            if (!empty($this->featuredImageData)) {
                $data['media']['featured_image'] = $this->featuredImageData;
            }

            if (!empty($this->galleryImagesData)) {
                $data['media']['gallery'] = $this->galleryImagesData;
            }
    ...
    // Media upload methods removed - now handled via JavaScript API calls

    /**
     * Set featured image data from JavaScript upload
     */
    public function setFeaturedImageData($imageData)
    {
        $this->featuredImageData = $imageData;
        $this->dispatch('uploadCompleted', [
            'type' => 'featuredImage',
            'message' => 'Immagine in evidenza aggiornata!'
        ]);
    }

    /**
     * Add gallery image data from JavaScript upload
     */
    public function addGalleryImageData($imageData)
    {
        $this->galleryImagesData[] = $imageData;
        $this->dispatch('uploadCompleted', [
            'type' => 'galleryImages',
            'message' => 'Immagine galleria aggiunta!'
        ]);
    }

    /**
     * Remove featured image data
     */
    public function removeFeaturedImageData()
    {
        $this->featuredImageData = [];
        $this->dispatch('uploadCompleted', [
            'type' => 'featuredImage',
            'message' => 'Immagine in evidenza rimossa!'
        ]);
    }
     ...
     */
    public function removeGalleryImageData($index)
    {
        if (isset($this->galleryImagesData[$index])) {
            unset($this->galleryImagesData[$index]);
            $this->galleryImagesData = array_values($this->galleryImagesData); // Reindex
            $this->dispatch('uploadCompleted', [
                'type' => 'galleryImages',
                'message' => 'Immagine galleria rimossa!'
            ]);
        }
    }
