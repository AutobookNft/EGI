...
class BiographyController extends Controller
{
    ...
    /**
     * Upload immagine da Trix editor (AJAX)
     *
     * @param Request $request
     * @return JsonResponse
     */
    public function uploadTrixImage(Request $request): JsonResponse
    {
        ...
        $user = auth()->user();
        $biography = null;
        if ($request->filled('biography_id')) {
            $biography = $user->biographies()->where('id', $request->biography_id)->firstOrFail();
        } else {
            // Se non c'è una biografia specifica, crea una temporanea o restituisci errore
            return response()->json(['success' => false, 'message' => 'Biografia non specificata'], 422);
        }

        /** @var UploadedFile $file */
        $file = $request->file('image');
        $media = $biography->addMedia($file)->toMediaCollection('main_gallery');
