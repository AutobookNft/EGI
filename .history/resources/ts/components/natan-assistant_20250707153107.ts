...
export class NatanAssistant {
    ...
    private async createButlerModal(): Promise<void> {
        ...
        butlerOptions.forEach(opt => {
            optionsHtml += `
                <button class="natan-butler-option natan-option-button" data-action="${opt.key}">
                    ...
                    <div>
                        <div style="color: #ecfdf5; font-weight: 500;">${t(opt.label)}</div>
                        <div style="color: rgba(110, 231, 183, 0.7); font-size: 0.75rem;">${t(opt.description)}</div>
                    </div>
                </button>
            `;
        });
        ...
        const headerTitle    = t('natan.header.title');
        const headerSubtitle = t('natan.header.subtitle');
        const welcomeMsg     = t('natan.welcome');
        const dismissLabel   = t('natan.dismiss');
        const closeAria      = t('natan.close_aria');
