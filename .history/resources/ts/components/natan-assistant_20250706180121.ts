...
export class NatanAssistant {
    ...
    // --- NUOVA STRUTTURA DATI OPZIONI ASSISTENTE ---
    ...
    const assistantOptions: AssistantOption[] = [
        {
            ...
            action: () => NatanAssistant.handleCreateArtwork()
        },
        {
            key: 'buy_artwork',
            label: getTranslation('assistant.buy_artwork'),
            action: () => NatanAssistant.handleBuyArtwork()
        },
        {
            key: 'what_is_egi',
            label: getTranslation('assistant.what_is_egi'),
            action: () => NatanAssistant.handleWhatIsEGI()
        },
        {
            key: 'guided_tour',
            label: getTranslation('assistant.guided_tour'),
            action: () => NatanAssistant.handleGuidedTour()
        },
        {
            key: 'custom_egi',
            label: getTranslation('assistant.custom_egi'),
            action: () => NatanAssistant.handleCustomEGI()
        },
        {
            key: 'white_paper',
            label: getTranslation('assistant.white_paper'),
            action: () => NatanAssistant.handleWhitePaper()
        },
        {
            key: 'let_me_guide',
            label: getTranslation('assistant.let_me_guide'),
            action: () => NatanAssistant.handleLetMeGuide()
        }
        // Altre opzioni future qui
    ];

    // --- METODI STATICI PER LE AZIONI ---
    static handleCreateArtwork() {
        // Logica per "Voglio creare le mie opere digitali o fisiche"
    }
    static handleBuyArtwork() {
        // Logica per "Voglio acquistare un'opera o un certificato"
    }
    static handleWhatIsEGI() {
        // Logica per "Voglio capire cos'è un EGI"
    }
    static handleGuidedTour() {
        // Logica per "Voglio fare un tour guidato della piattaforma"
    }
    static handleCustomEGI() {
        // Logica per "Voglio creare un mio EGI personalizzato"
    }
    static handleWhitePaper() {
        // Logica per "Voglio leggere il White Paper"
    }
    static handleLetMeGuide() {
        // Logica per "Non lo so… accompagnami tu"
    }
}
