...
export class NatanAssistant {
    ...
    private async initButlerModalAsync() {
        await this.createButlerModal();
    }
     ...
     */
    private async createButlerModal(): Promise<void> {
        ...
        let optionsHtml = '';
        butlerOptions.forEach(opt => {
            optionsHtml += `
                <button class="natan-butler-option natan-option-button" data-action="${opt.key}">
                    <span style="font-size: 1.25rem;">${opt.icon}</span>
                    <div>
                        <div style="color: #ecfdf5; font-weight: 500;">${getTranslation(opt.label)}</div>
                        <div style="color: rgba(110, 231, 183, 0.7); font-size: 0.75rem;">${getTranslation(opt.description)}</div>
                    </div>
                </button>
            `;
        });

        // --- INIZIO LOCALIZZAZIONE STRINGHE BUTLER ---
        const t = getTranslation;
        const headerTitle    = t('natan.header.title');      // "Ciao! Sono Natan"
        const headerSubtitle = t('natan.header.subtitle');   // "Il tuo maggiordomo digitale"
        const welcomeMsg     = t('natan.welcome');           // "Benvenuto! Sono qui per aiutarti a navigare nel mondo EGI. Cosa ti interessa scoprire?"
        const dismissLabel   = t('natan.dismiss');           // "Non mostrare più questo messaggio"
        const closeAria      = t('natan.close_aria');        // "Chiudi assistente"
        ...
        this.butlerModal.innerHTML = `
            ...
            ">
                ...
                ">
                    ...
                    <p style="color: #6ee7b7; font-size: 0.875rem;">${headerSubtitle}</p>
                </div>

                <!-- Messaggio di benvenuto -->
                <div class="natan-content">
                    <div class="natan-welcome-message">
                        <span class="natan-welcome-text">${welcomeMsg}</span>
                    </div>
    ...
    private setupButlerEventListeners(): void {
        ...
        this.butlerModal.querySelectorAll('.natan-butler-option').forEach(optionBtn => {
            optionBtn.addEventListener('click', (e) => {
                const key = (e.currentTarget as HTMLElement).getAttribute('data-action');
                if (key) {
                    // Trova la funzione associata
                    const opt = butlerOptions.find(o => o.key === key);
...
import { ensureTranslationsLoaded, getTranslation } from '../../js/utils/translations';
