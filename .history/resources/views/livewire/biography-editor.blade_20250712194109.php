...
@push('scripts')
    ...
    <script>
            ...
            return {
                ...
                initTrix() {
                    this.$nextTick(() => {
                        ...
                        // Watch for content changes from Livewire (when loading existing biography)
                        this.$watch('content', (newContent) => {
                            if (newContent && newContent !== trixEditor.getDocument().toString()) {
                                console.log('Updating Trix editor with loaded content:', newContent);
                                trixEditor.loadHTML(newContent);
                            }
                        });
                    });
                },
            ...
            return {
                ...
                initChapterTrix() {
                    this.$nextTick(() => {
                        ...
                        // Watch for content changes from Livewire (when loading existing chapter)
                        this.$watch('chapterContent', (newContent) => {
                            if (newContent && newContent !== trixEditor.getDocument().toString()) {
                                console.log('Updating Chapter Trix editor with loaded content:',
                                    newContent);
                                trixEditor.loadHTML(newContent);
                            }
                        });
        ...
        // Media Upload Manager - Pure JavaScript Solution
        function mediaUploadManager() {
            return {
                ...
                biographyId: @this.get('biographyId') || null,
                ...
                // Load existing images from Livewire
                loadExistingImages() {
                    this.allImages = [];
                ...
                setupEventListeners() {
                    // Listen for biography ID updates from Livewire
                    document.addEventListener('livewire:initialized', () => {
                        @this.on('biographyIdUpdated', (event) => {
                            this.biographyId = event.biographyId;
                        });

                        // Listen for biography saved event to reload images
                        @this.on('biographySaved', (event) => {
                            this.loadExistingImages();
                        });
                    });
                ...
                // Upload multiple images - Pure JavaScript
                async handleMultipleImagesUpload(event) {
                    ...
                    try {
                        for (const file of files) {
                            ...
                            if (this.biographyId) {
                                formData.append('biography_id', this.biographyId);
                            }
                            ...
                            if (result.success) {
                                ...
                                // Update biography ID if it was created
                                if (result.media.biography_id && !this.biographyId) {
                                    this.biographyId = result.media.biography_id;
                                    @this.call('setBiographyId', this.biographyId);
                                }

                                this.showNotification('success', 'Immagine caricata con successo');
                            } else {
                                this.multipleImagesError = result.message || 'Errore durante l\'upload';
                                this.showNotification('error', result.message || 'Errore durante l\'upload');
                            }
                        }
                    } catch (error) {
                        console.error('Upload error:', error);
                        this.multipleImagesError = 'Errore di connessione durante l\'upload';
                        this.showNotification('error', 'Errore di connessione durante l\'upload');
                    } finally {
                ...
                async removeImage(index) {
                    ...
                    try {
                        ...
                        if (result.success) {
                            ...
                            this.showNotification('success', 'Immagine rimossa con successo');
                        } else {
                            this.showNotification('error', result.message || 'Errore durante la rimozione');
                        }
                    } catch (error) {
                        console.error('Remove error:', error);
                        this.showNotification('error', 'Errore di connessione durante la rimozione');
                    }
                },
                ...
                async updateAvatar(imageUrl) {
                    ...
                    try {
                        ...
                        this.showNotification('error', 'Errore durante l\'aggiornamento dell\'avatar');
                    }
                },
