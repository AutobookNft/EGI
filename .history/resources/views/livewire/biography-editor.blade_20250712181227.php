<div class="space-y-6">
    ...
    <div class="flex items-center justify-between border-t border-gray-700 pt-6">
        ...
        <div class="flex items-center space-x-4">
            ...
            <button onclick="handleSave()" wire:loading.attr="disabled"
...
@push('scripts')
    ...
    <script>
                ...
                initChapterTrix() {
                    this.$nextTick(() => {
                        ...
                        this.$watch('chapterContent', (newContent) => {
                            if (newContent && newContent !== trixEditor.getDocument().toString()) {
                                console.log('Updating Chapter Trix editor with loaded content:', newContent);
                                trixEditor.loadHTML(newContent);
                            }
                        });
        ...
        // Function to handle saving the biography
        function handleSave() {
            // Sync content from Trix editors before saving
            const contentEditor = document.getElementById('content-trix');
            const chapterContentEditor = document.getElementById('chapter-content-trix');

            // Sync main content
            if (contentEditor && contentEditor.editor) {
                const contentToSave = contentEditor.editor.getDocument().toString();
                @this.call('updateTrixContent', contentToSave);
            }

            // Sync chapter content if available
            if (chapterContentEditor && chapterContentEditor.editor) {
                const chapterContentToSave = chapterContentEditor.editor.getDocument().toString();
                @this.call('updateChapterTrixContent', chapterContentToSave);
            }

            // Small delay to ensure content is synced, then save
            setTimeout(() => {
                @this.call('save');
            }, 100);
        }
