            ...
            return {
                ...
                loadExistingImages() {
                    ...
                    @if($featuredImageData && !empty($featuredImageData))
                        this.allImages.push({
                            id: 'existing_featured',
                            collection_name: 'featured_image',
                            file_name: '{{ $featuredImageData['filename'] ?? 'featured.jpg' }}',
                            url: '{{ $featuredImageData['url'] ?? '' }}',
                            size: {{ $featuredImageData['size'] ?? 0 }},
                            is_existing: true
                        });
                        this.selectedFeaturedImageId = 'existing_featured';
                    @endif

                    // Add gallery images if exist
                    @if($galleryImagesData && !empty($galleryImagesData))
                        @foreach($galleryImagesData as $index => $image)
                            this.allImages.push({
                                id: 'existing_gallery_{{ $index }}',
                                collection_name: 'main_gallery',
                                file_name: '{{ $image['filename'] ?? 'gallery.jpg' }}',
                                url: '{{ $image['url'] ?? '' }}',
                                size: {{ $image['size'] ?? 0 }},
                                is_existing: true
                            });
