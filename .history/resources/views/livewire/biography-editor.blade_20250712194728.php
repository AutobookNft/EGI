...
@push('scripts')
    ...
    <script>
        ...
        // ==========================================
        // JAVASCRIPT SEMPLIFICATO E FUNZIONANTE
        // ==========================================

        document.addEventListener('DOMContentLoaded', function() {
            console.log('Inizializzazione JavaScript...');

            // Editor Trix semplificato
            setupTrixEditor();

            // Media upload semplificato
            setupMediaUpload();
        });

        function setupTrixEditor() {
            // Editor principale
            const contentEditor = document.querySelector('trix-editor[input="content-trix"]');
            if (contentEditor) {
                const hiddenInput = document.getElementById('content-trix');

                contentEditor.addEventListener('trix-change', function(event) {
                    const content = event.target.editor.getDocument().toString();
                    hiddenInput.value = content;

                    // Informa Livewire del cambiamento
                    hiddenInput.dispatchEvent(new Event('input', {
                        bubbles: true
                    }));
                });

                console.log('Editor Trix principale configurato');
            }

            // Editor capitoli
            const chapterEditor = document.querySelector('trix-editor[input="chapter-content-trix"]');
            if (chapterEditor) {
                const chapterInput = document.getElementById('chapter-content-trix');

                chapterEditor.addEventListener('trix-change', function(event) {
                    const content = event.target.editor.getDocument().toString();
                    chapterInput.value = content;
                ...
                console.log('Editor Trix capitoli configurato');
            }
        }

        // Upload media semplificato
        function setupMediaUpload() {
            ...
            fileInput.addEventListener('change', async function(event) {
                ...
                // Ricarica la pagina per mostrare le immagini
                setTimeout(() => {
        ...
        async function uploadSingleFile(file) {
            ...
            if (file.size > 2 * 1024 * 1024) {
                throw new Error(`${file.name} supera i 2MB`);
            }
            ...
            console.log('File caricato:', result.media.file_name);
            showSuccess('Immagine caricata: ' + result.media.file_name);
        ...
        function showLoading() {
            let loading = document.getElementById('upload-loading');
            if (loading) {
