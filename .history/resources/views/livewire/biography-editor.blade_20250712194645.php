...
@push('scripts')
    ...
    <script>
        ...
        // ==========================================
        // JAVASCRIPT PURO - NIENTE ALPINE.JS
        // ==========================================

        // Gestione Editor Trix - JavaScript Puro
        document.addEventListener('DOMContentLoaded', function() {
            initializeTrixEditor();
            initializeMediaUpload();
            initializeChapterTrixEditor();
        });

        function initializeTrixEditor() {
            const trixEditor = document.querySelector('#content-trix');
            if (!trixEditor) return;

            const editor = trixEditor.nextElementSibling;
            if (!editor) return;

            // Sincronizza contenuto con Livewire
            ...
            if (existingContent && editor.editor) {
                editor.editor.loadHTML(existingContent);
            }
        }

        function initializeChapterTrixEditor() {
            const chapterTrixEditor = document.querySelector('#chapter-content-trix');
            if (!chapterTrixEditor) return;

            const editor = chapterTrixEditor.nextElementSibling;
            if (!editor) return;

            // Sincronizza contenuto con Livewire
            editor.addEventListener('trix-change', function(event) {
                const content = event.target.editor.getDocument().toString();
                @this.call('updateChapterTrixContent', content);
            });
        }

        // Gestione Media Upload - JavaScript Puro
        ...
        function initializeMediaUpload() {
            ...
            // Inizializza stato
            mediaUploadState.biographyId = @this.get('biographyId') || null;
            loadExistingImages();

            // Event listener per upload
            fileInput.addEventListener('change', handleFileUpload);

            // Event listener per eventi Livewire
            document.addEventListener('livewire:initialized', () => {
                @this.on('biographyIdUpdated', (event) => {
                    mediaUploadState.biographyId = event.biographyId;
                });
            ...
            @if ($mediaData && !empty($mediaData))
                @foreach ($mediaData as $index => $media)
                    ...
                    @if ($media['collection_name'] === 'featured_image')
                        if (!mediaUploadState.selectedFeaturedImageId) {
                            mediaUploadState.selectedFeaturedImageId = '{{ $media['id'] }}';
                        }
                    @endif
                @endforeach
            @endif
            ...
            try {
                for (const file of files) {
                    ...
                    if (!file.type.startsWith('image/')) {
                        setError(`Il file ${file.name} non è un'immagine`);
                        continue;
                    }

                    if (file.size > 2 * 1024 * 1024) { // 2MB
                        setError(`L'immagine ${file.name} supera i 2MB`);
                        continue;
                    }
                    ...
                    if (mediaUploadState.biographyId) {
                        formData.append('biography_id', mediaUploadState.biographyId);
                    }
                    ...
                    if (result.success) {
                        ...
                        mediaUploadState.images.push(result.media);

                        // Aggiorna biography ID se creato
                        if (result.media.biography_id && !mediaUploadState.biographyId) {
                            mediaUploadState.biographyId = result.media.biography_id;
                            @this.call('setBiographyId', mediaUploadState.biographyId);
                        }
        ...
        async function removeImage(index) {
            const image = mediaUploadState.images[index];

            try {
                const response = await fetch('{{ route('biography.remove-media') }}', {
                    method: 'DELETE',
                    headers: {
                ...
                if (result.success) {
                    // Rimuovi dall'array
                    mediaUploadState.images.splice(index, 1);

                    // Aggiorna selezioni
                    if (mediaUploadState.selectedFeaturedImageId === image.id) {
                        mediaUploadState.selectedFeaturedImageId = '';
                    }
                    if (mediaUploadState.selectedAvatarUrl === image.url) {
                        mediaUploadState.selectedAvatarUrl = '';
                    }

                    renderImagesGallery();
                    showNotification('success', 'Immagine rimossa con successo');
                } else {
                    showNotification('error', result.message || 'Errore durante la rimozione');
                }
            } catch (error) {
        ...
        function renderImagesGallery() {
            ...
            if (mediaUploadState.images.length === 0) {
            ...
            mediaUploadState.images.forEach((image, index) => {
                const imageCard = document.createElement('div');
                imageCard.className = 'space-y-3 rounded-lg border border-gray-600 bg-gray-800/50 p-4';

                imageCard.innerHTML = `
                    <div class="relative">
                        <img src="${image.url}" alt="Image ${index + 1}" class="h-32 w-full rounded-lg object-cover">
                        <button onclick="removeImage(${index})" class="absolute -right-2 -top-2 flex h-6 w-6 items-center justify-center rounded-full bg-red-500 text-white transition-colors hover:bg-red-600">
                    ...
                    <div class="space-y-3">
                        <div class="flex items-center space-x-2">
                            <input type="radio" id="featured-${image.id}" name="featured_image" value="${image.id}"
                                   ${mediaUploadState.selectedFeaturedImageId === image.id ? 'checked' : ''}
                                   class="h-4 w-4 border-gray-600 bg-gray-700 text-[#D4A574] focus:ring-2 focus:ring-[#D4A574]">
                            <label for="featured-${image.id}" class="cursor-pointer text-sm font-medium text-gray-300">
                        ...
                        <div class="flex items-center space-x-2">
                            <input type="radio" id="avatar-${image.id}" name="avatar_selection" value="${image.url}"
                                   ${mediaUploadState.selectedAvatarUrl === image.url ? 'checked' : ''}
                                   onchange="updateAvatar(this.value)"
                                   class="h-4 w-4 border-gray-600 bg-gray-700 text-[#D4A574] focus:ring-2 focus:ring-[#D4A574]">
                            <label for="avatar-${image.id}" class="cursor-pointer text-sm font-medium text-gray-300">
                                <span class="flex items-center space-x-2">
                                    <svg class="h-4 w-4 text-[#D4A574]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        async function updateAvatar(imageUrl) {
            ...
            try {
                ...
                if (result.success) {
                    mediaUploadState.selectedAvatarUrl = imageUrl;
                    showNotification('success', result.message);
                } else {
                    showNotification('error', result.message || 'Errore durante l\'aggiornamento dell\'avatar');
                }
            } catch (error) {
                console.error('Avatar update error:', error);
                showNotification('error', 'Errore durante l\'aggiornamento dell\'avatar');
            }
        }

        // Utility functions
        function setLoading(loading) {
            mediaUploadState.loading = loading;
        ...
        window.addEventListener('biography-load', function(event) {
            ...
            // Delay to ensure Trix editor is ready, then sync content
            setTimeout(() => {
                syncTrixContent();
            }, 500);
        });

        // Function to sync Trix content with Livewire data
        function syncTrixContent() {
            // Sync main content to Trix editor
            const contentEditor = document.getElementById('content-trix');
            if (contentEditor && contentEditor.editor) {
                const content = @this.get('content') || '';
                console.log('Syncing main Trix content:', content);
                contentEditor.editor.loadHTML(content);
            }

            // Sync chapter content if editing chapter
            const chapterEditor = document.getElementById('chapter-content-trix');
            if (chapterEditor && chapterEditor.editor) {
                const chapterContent = @this.get('currentChapter.content') || '';
                console.log('Syncing chapter Trix content:', chapterContent);
                chapterEditor.editor.loadHTML(chapterContent);
            }
        }

        // Listen for content loaded event to sync Trix editor
        document.addEventListener('livewire:load', function() {
            @this.on('contentLoaded', (data) => {
                console.log('Content loaded event received', data);

                // Sync main content to Trix editor
                const contentEditor = document.getElementById('content-trix');
                if (contentEditor && contentEditor.editor) {
                    const content = @this.get('content') || '';
                    console.log('Setting Trix content:', content);
                    contentEditor.editor.loadHTML(content);
                }

                // Sync chapter content if in chapter editing mode
                const chapterEditor = document.getElementById('chapter-content-trix');
                if (chapterEditor && chapterEditor.editor) {
                    const chapterContent = @this.get('currentChapter.content') || '';
                    console.log('Setting chapter Trix content:', chapterContent);
                    chapterEditor.editor.loadHTML(chapterContent);
                }
            });
        });
