        ...
        @if ($activeTab === 'media')
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <div class="space-y-4">
                        <input type="file" wire:model.defer="featuredImage" accept="image/*"
                        ...
                        <div wire:loading wire:target="featuredImage"
                            class="flex items-center space-x-2 text-[#D4A574]">
                            <div class="h-4 w-4 animate-spin rounded-full border-b-2 border-[#D4A574]"></div>
                            <span class="text-sm">Caricamento immagine...</span>
                        </div>
                ...
                <div>
                    ...
                    <input type="file" wire:model.defer="galleryImages" multiple accept="image/*"
                        class="w-full rounded-lg border border-gray-600 bg-gray-800 px-4 py-3 text-white transition-colors file:mr-4 file:rounded-lg file:border-0 file:bg-[#D4A574] file:px-4 file:py-2 file:text-sm file:font-medium file:text-gray-900 hover:file:bg-[#E6B885]"
                    ...
                    <div wire:loading wire:target="galleryImages"
                        class="mt-2 flex items-center space-x-2 text-[#D4A574]">
                        <div class="h-4 w-4 animate-spin rounded-full border-b-2 border-[#D4A574]"></div>
                        <span class="text-sm">Caricamento immagini...</span>
                    </div>
...
function checkFileSize(input, maxSizeKB) {
    if (input.files && input.files[0]) {
        ...
        if (fileSize > maxSizeKB) {
            alert(`Il file è troppo grande (${Math.round(fileSize)} KB). Dimensione massima consentita: ${maxSizeKB} KB (${Math.round(maxSizeKB/1024)} MB)`);
...
function checkMultipleFileSize(input, maxSizeKB) {
    if (input.files && input.files.length > 0) {
        for (let i = 0; i < input.files.length; i++) {
            ...
            if (fileSize > maxSizeKB) {
                alert(`Il file "${input.files[i].name}" è troppo grande (${Math.round(fileSize)} KB). Dimensione massima consentita: ${maxSizeKB} KB (${Math.round(maxSizeKB/1024)} MB)`);
                input.value = ''; // Clear the input
                return false;
            }
        }
