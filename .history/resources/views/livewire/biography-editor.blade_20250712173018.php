<div class="space-y-6">
            ...
            <div class="space-y-6" x-data="mediaUploadManager()">
                ...
                <div>
                    ...
                    <div class="space-y-4">
                        ...
                        <!-- Loading indicator -->
                        <div x-show="featuredImageLoading" x-transition
                        ...
                        <!-- Error message -->
                        <div x-show="featuredImageError" x-transition class="text-sm text-red-400" x-text="featuredImageError"></div>

                        <!-- Featured Image Preview -->
                        <div x-show="featuredImagePreview" x-transition class="mt-4 space-y-3">
                            ...
                            <!-- Avatar Checkbox for Featured Image -->
                            <div class="flex items-center space-x-2">
                                <input type="radio"
                                    :id="'avatar-featured'"
                                    name="avatar_selection"
                                    :value="featuredImageData?.url || ''"
                                    x-model="selectedAvatarUrl"
                                    @change="updateAvatar($event.target.value)"
                                    class="h-4 w-4 text-[#D4A574] bg-gray-700 border-gray-600 focus:ring-[#D4A574] focus:ring-2">
                                <label :for="'avatar-featured'" class="text-sm font-medium text-gray-300 cursor-pointer">
                ...
                <div>
                    ...
                    <input type="file" id="gallery-images-input" multiple accept="image/*"
                        class="w-full rounded-lg border border-gray-600 bg-gray-800 px-4 py-3 text-white transition-colors file:mr-4 file:rounded-lg file:border-0 file:bg-[#D4A574] file:px-4 file:py-2 file:text-sm file:font-medium file:text-gray-900 hover:file:bg-[#E6B885]"
                        @change="handleGalleryImagesUpload($event)">

                    <!-- Loading indicator -->
                    <div x-show="galleryImagesLoading" x-transition
                    ...
                    <!-- Error message -->
                    <div x-show="galleryImagesError" x-transition class="mt-1 text-sm text-red-400" x-text="galleryImagesError"></div>

                    <!-- Gallery Images Preview -->
                    <div x-show="galleryImagesPreviews.length > 0" x-transition class="mt-4 grid grid-cols-1 gap-6 md:grid-cols-2">
                        <template x-for="(image, index) in galleryImagesPreviews" :key="index">
                            <div class="space-y-3">
                                <div class="relative">
                                    <img :src="image.preview" alt="Gallery preview"
                                ...
                                <!-- Avatar Checkbox for Gallery Image -->
                                <div class="flex items-center space-x-2">
                                    <input type="radio"
                                        :id="'avatar-gallery-' + index"
                                        name="avatar_selection"
                                        :value="galleryImagesData[index]?.url || ''"
                                        x-model="selectedAvatarUrl"
                                        @change="updateAvatar($event.target.value)"
                                        class="h-4 w-4 text-[#D4A574] bg-gray-700 border-gray-600 focus:ring-[#D4A574] focus:ring-2">
                                    <label :for="'avatar-gallery-' + index" class="text-sm font-medium text-gray-300 cursor-pointer">
                                        <span class="flex items-center space-x-2">
                                            <svg class="h-4 w-4 text-[#D4A574]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path>
                                            </svg>
    ...
    <div class="flex items-center justify-between border-t border-gray-700 pt-6">
        <div class="flex items-center space-x-4">
            ...
            <div class="unsaved-indicator hidden items-center space-x-2 text-yellow-400 text-sm">
        ...
        <div class="flex items-center space-x-4">
            ...
            <button wire:click="resetUploadState"
                    class="hidden emergency-reset inline-flex items-center rounded-lg bg-yellow-600 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-yellow-700"
...
@push('scripts')
    ...
    <script>
        ...
        document.addEventListener('livewire:load', function() {
            ...
            Livewire.hook('request', ({ uri, options, payload, respond, succeed, fail }) => {
                ...
                if (requestCount > 5) {
                    ...
                    fail({ status: 429, message: 'Too many requests' });
                    return;
                }
            ...
            Livewire.hook('response', ({ request, response }) => {
            ...
            Livewire.hook('failed', ({ request, response }) => {
        ...
        document.addEventListener('livewire:load', function() {
            ...
            window.addEventListener('livewire:upload-start', (event) => {
                ...
                uploadTimeout = setTimeout(() => {
                    ...
                    notification.textContent = 'Upload timeout - riprova con un\'immagine più piccola';
                    document.body.appendChild(notification);
            ...
            window.addEventListener('livewire:upload-error', (event) => {
             ...
             setInterval(() => {
                 if (uploadInProgress) {
                     const visibleLoaders = document.querySelectorAll('[wire\\:loading]:not([style*="display: none"])');
        ...
        // Media Upload Manager with JavaScript (no Livewire dependency)
        function mediaUploadManager() {
            return {
                ...
                init() {
                    ...
                    @if($featuredImageData && !empty($featuredImageData))
                        this.featuredImageData = @json($featuredImageData);
                        this.featuredImagePreview = '{{ $featuredImageData['url'] ?? '' }}';
                    @endif

                    // Load existing gallery images if available
                    @if($galleryImagesData && !empty($galleryImagesData))
                        this.galleryImagesData = @json($galleryImagesData);
                        this.galleryImagesPreviews = @json($galleryImagesData).map(img => ({
                            preview: img.url,
                            name: img.filename
                        }));
                    @endif
                ...
                async handleFeaturedImageUpload(event) {
                    ...
                    if (!file.type.startsWith('image/')) {
                        this.featuredImageError = 'Il file deve essere un\'immagine';
                        this.featuredImageLoading = false;
                        return;
                    }

                    if (file.size > 2 * 1024 * 1024) { // 2MB
                        this.featuredImageError = 'L\'immagine non può superare i 2MB';
                        this.featuredImageLoading = false;
                        return;
                    }
                    ...
                    try {
                        ...
                        if (result.success) {
                            this.featuredImageData = result.file;
                            this.showNotification('success', result.message);

                            // Update Livewire component with file data
                            @this.call('setFeaturedImageData', result.file);
                        } else {
                            this.featuredImageError = result.message || 'Errore durante l\'upload';
                            this.featuredImagePreview = '';
                            this.featuredImageData = null;
                        }
                    } catch (error) {
                        console.error('Upload error:', error);
                        this.featuredImageError = 'Errore di connessione durante l\'upload';
                        this.featuredImagePreview = '';
                        this.featuredImageData = null;
                    } finally {
                        this.featuredImageLoading = false;
                ...
                removeFeaturedImage() {
                    ...
                    @this.call('removeFeaturedImageData');

                    this.showNotification('success', 'Immagine in evidenza rimossa');
                },
                ...
                async handleGalleryImagesUpload(event) {
                    ...
                    this.galleryImagesLoading = true;

                    try {
                        for (const file of files) {
                            // Client-side validation
                            if (!file.type.startsWith('image/')) {
                                this.galleryImagesError = `Il file ${file.name} non è un'immagine`;
                                continue;
                            }

                            if (file.size > 2 * 1024 * 1024) { // 2MB
                                this.galleryImagesError = `L'immagine ${file.name} supera i 2MB`;
                                continue;
                            }
                            ...
                            const reader = new FileReader();
                            reader.onload = (e) => {
                                this.galleryImagesPreviews.push({
                            ...
                            if (result.success) {
                                this.galleryImagesData.push(result.file);

                                // Update Livewire component with file data
                                @this.call('addGalleryImageData', result.file);
                            } else {
                                this.galleryImagesError = result.message || 'Errore durante l\'upload';
                            }
                        }

                        if (this.galleryImagesError === '') {
                            this.showNotification('success', `${files.length} immagini caricate con successo`);
                        }
                    } catch (error) {
                        console.error('Upload error:', error);
                        this.galleryImagesError = 'Errore di connessione durante l\'upload';
                    } finally {
                        this.galleryImagesLoading = false;
                        event.target.value = ''; // Reset input
                    }
                },
                ...
                removeGalleryImage(index) {
                    ...
                    // Update Livewire component
                    @this.call('removeGalleryImageData', index);

                    this.showNotification('success', 'Immagine rimossa dalla galleria');
                },
                ...
                async updateAvatar(imageUrl) {
                    ...
                    try {
                        ...
                        if (result.success) {
                            ...
                            this.showNotification('error', result.message || 'Errore durante l\'aggiornamento dell\'avatar');
                        }
                    } catch (error) {
