<x-app-layout>
    ...
    <div class="tab-panels space-y-8">
        ...
        <div id="profile-images-panel"
             class="hidden tab-panel"
             role="tabpanel"
             aria-labelledby="profile-images-tab"
             aria-hidden="true">
            ...
            <div class="space-y-6">
                    ...
                    @if(auth()->user()->getCurrentProfileImage())
                        <div class="flex items-center space-x-4 mb-4">
                            <img src="{{ auth()->user()->getCurrentProfileImage()->getUrl('avatar') }}"
                                 ...
                                 class="w-20 h-20 rounded-full object-cover border-2 border-oro-fiorentino">
                            <div>
                                <p class="text-sm gdpr-subtitle">{{ __('profile.currently_active') }}</p>
                                <p class="text-xs text-gray-500">{{ auth()->user()->getCurrentProfileImage()->created_at->format('d M Y H:i') }}</p>
                            </div>
                        </div>
                    @else
                        <div class="text-center py-8">
                            ...
                            <p class="text-sm gdpr-subtitle">{{ __('profile.no_current_image') }}</p>
                        </div>
                    @endif
                </div>
                ...
                <div class="p-6 gdpr-card rounded-2xl">
                    ...
                    <form action="{{ route('profile.upload-image') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                        @csrf
                        <div class="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center">
                            ...
                            <label for="profile_image" class="cursor-pointer block">
                                ...
                                <p class="text-sm gdpr-subtitle">{{ __('profile.click_to_upload') }}</p>
                                <p class="text-xs text-gray-500 mt-1">{{ __('profile.supported_formats') }}</p>
                            </label>
                        </div>
                        <button type="submit" class="w-full inline-flex items-center justify-center px-4 py-2 text-sm rounded-lg gdpr-btn-primary">
                ...
                <div class="p-6 gdpr-card rounded-2xl">
                    <div class="flex items-center justify-between mb-6">
                        ...
                        <span class="text-sm text-gray-500">{{ auth()->user()->getAllProfileImages()->count() }} {{ __('profile.images') }}</span>
                    </div>
                        ...
                        <div class="relative group">
                            ...
                            <div class="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-50 transition-all duration-200 rounded-lg flex items-center justify-center">
                                <div class="opacity-0 group-hover:opacity-100 transition-opacity duration-200 space-x-2">
                                    @if(!auth()->user()->getCurrentProfileImage() || auth()->user()->getCurrentProfileImage()->id !== $image->id)
                                    <form action="{{ route('profile.set-current-image') }}" method="POST" class="inline">
                                        @csrf
                                    ...
                                    @endif
                                    <form action="{{ route('profile.delete-image') }}" method="POST" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <input type="hidden" name="media_id" value="{{ $image->id }}">
