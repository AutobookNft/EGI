<x-guest-layout>
    <x-slot name="title">{{ $biography ? $biography->title . ' - ' . $user->name : __('biography.show.title') }}</x-slot>
    <x-slot name="metaDescription">{{ $biography ? $biography->excerpt : __('biography.show.no_biography_description') }}</x-slot>
    ...
    @if($biography)
                    ...
                    <div class="mb-8">
                        @if($user->profile_photo_url)
                            <img src="{{ $user->profile_photo_url }}" alt="{{ $user->name }}"
                                 class="w-24 h-24 rounded-full border-4 border-[#D4A574] mx-auto mb-4">
                        @else
                            <div class="w-24 h-24 bg-[#D4A574] rounded-full flex items-center justify-center mx-auto mb-4">
                                <span class="text-3xl font-bold text-gray-900">{{ substr($user->name, 0, 1) }}</span>
                            </div>
                        @endif
                        <h2 class="mb-2 text-2xl font-semibold text-white">{{ $user->name }}</h2>
                        @if($user->bio)
                            <p class="text-[#D4A574] max-w-2xl mx-auto">{{ $user->bio }}</p>
                        @endif
                    </div>
        ...
        <div class="max-w-4xl px-4 py-12 mx-auto sm:px-6 lg:px-8">
            @if($biography->type === 'single')
                ...
                <div class="p-8 mb-8 border border-gray-700 bg-gray-800/50 backdrop-blur-sm rounded-xl">
            ...
            @else
                ...
                <div class="space-y-8">
                    @foreach($biography->chapters as $chapter)
                        <div class="overflow-hidden border border-gray-700 bg-gray-800/50 backdrop-blur-sm rounded-xl">
                            <!-- Chapter Header -->
                            <div class="p-6 border-b border-gray-700">
                                ...
                                @if($chapter->subtitle)
                                    <p class="text-[#D4A574] text-lg mb-4">
                                        {{ $chapter->subtitle }}
                                    </p>
                                @endif
            ...
            <!-- Author CTA -->
            <div class="mt-12 bg-gradient-to-r from-[#1B365D]/50 to-[#2D5016]/50 border border-gray-700 rounded-xl p-8 text-center">
                ...
                <div class="flex flex-col justify-center space-y-4 sm:flex-row sm:space-y-0 sm:space-x-4">
                    <a href="{{ route('home.creator.home', $user->id) }}"
                       class="inline-flex items-center justify-center px-6 py-3 bg-gradient-to-r from-[#D4A574] to-[#E6B885] text-gray-900 font-semibold rounded-lg hover:from-[#E6B885] hover:to-[#D4A574] transition-all duration-200">
    ...
    @else
        ...
        <div class="flex items-center justify-center min-h-screen">
            <div class="text-center">
                ...
                <a href="{{ route('home.creator.show', $user->slug) }}"
