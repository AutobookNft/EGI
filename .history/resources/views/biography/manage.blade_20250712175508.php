    ...
    <div id="biographyModal" class="fixed inset-0 z-50 hidden bg-black bg-opacity-50 backdrop-blur-sm">
        <div class="flex min-h-screen items-center justify-center p-4">
            <div class="max-h-[90vh] w-full max-w-4xl overflow-y-auto rounded-xl bg-gray-800 shadow-2xl">
                ...
                <!-- Modal Content -->
                <div class="p-6">
                    <livewire:biography-editor wire:id="biography-editor" />
                </div>
            </div>
        </div>
    </div>
    ...
    @push('scripts')
        <script>
            ...
            function createNewBiography() {
                ...
                // Reset form after modal opens - dispatch event
                setTimeout(() => {
                    window.dispatchEvent(new CustomEvent('biography-reset-form'));
                }, 100);
            }
            ...
            function editBiography(biographyId) {
                ...
                // Load biography data after modal opens - dispatch event
                setTimeout(() => {
                    window.dispatchEvent(new CustomEvent('biography-load', {
                        detail: {
            ...
            function deleteBiography(biographyId) {
                ...
                deleteButton.innerHTML = '<div class="h-4 w-4 animate-spin rounded-full border-b-2 border-red-400"></div>';
                deleteButton.disabled = true;

                fetch(`/api/biographies/${biographyId}`, {
                        method: 'DELETE',
                        headers: {
                            'Authorization': 'Bearer {{ auth()->user()->createToken('biography-delete')->plainTextToken ?? '' }}',
