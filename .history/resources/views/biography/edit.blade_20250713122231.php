<x-app-layout>
    ...
    <div class="max-w-4xl px-4 py-12 mx-auto sm:px-6 lg:px-8">
        <div class="p-8 border border-gray-700 shadow-2xl rounded-xl bg-gray-800/50 backdrop-blur-sm">
            ...
            <!-- Form -->
            <form action="{{ isset($biography) ? route('biography.update', $biography->id) : route('biography.store') }}" method="POST" enctype="multipart/form-data" id="biography-form">
                @csrf
                @if(isset($biography))
                    @method('PUT')
                @endif
                ...
                <div id="basic-tab" class="space-y-6">
                    ...
                    <!-- Type -->
                    <div>
                        <label for="type" class="block mb-2 text-sm font-medium text-gray-300">Tipo *</label>
                        <select id="type" name="type"
                            ...
                            <option value="single" {{ old('type') == 'single' ? 'selected' : '' }}>Biografia Singola
                            </option>
                            <option value="chapters" {{ old('type') == 'chapters' ? 'selected' : '' }}>Biografia a
                                Capitoli</option>
                        </select>
                        @error('type')
                            <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
