    ...
    <div class="relative flex min-h-screen items-center justify-center p-4">
        <div
            ...
            <div class="max-h-[calc(90vh-120px)] overflow-y-auto p-6">
                <form id="biography-form" class="space-y-6" method="POST" action="{{ route('biography.store') }}"
                    ...
                    <div>
                        <label for="featured_image" class="mb-2 block text-sm font-medium text-gray-300">
                            {{ __('biography.form.featured_image') }}
                        </label>
                        <input type="file" id="featured_image" name="featured_image"
                            accept="image/jpeg,image/png,image/webp"
                            class="w-full rounded-lg border border-gray-600 bg-gray-800 px-4 py-3 text-white placeholder-gray-400 transition-colors focus:border-[#D4A574] focus:ring-1 focus:ring-[#D4A574]">
                        <p class="mt-1 text-xs text-gray-400">{{ __('biography.form.featured_image_hint') }}</p>
                    </div>
...
@push('scripts')
    <script>
        ...
        function uploadTrixImage(attachment) {
            ...
            // Se vuoi passare anche l'id biografia:
            const bioId = document.getElementById('biography-id').value;
            if (bioId) formData.append('biography_id', bioId);

            fetch('/api/biographies/trix-image', {
                method: 'POST',
                headers: {
            ...
            .then(data => {
                if (data.success && data.url) {
                    ...
                    alert('Errore upload immagine: ' + (data.message || ''));
                }
            })
            .catch(() => {
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.getElementById('create-biography-btn')?.addEventListener('click', function() {
            document.getElementById('biography-form').reset();
            document.getElementById('biography-id').value = '';
            document.querySelector('trix-editor[input="content"]').editor.loadHTML('');
            document.getElementById('biography-modal').classList.remove('hidden');
        });
        document.getElementById('create-first-biography-btn')?.addEventListener('click', function() {
            document.getElementById('biography-form').reset();
            document.getElementById('biography-id').value = '';
            document.querySelector('trix-editor[input="content"]').editor.loadHTML('');
            document.getElementById('biography-modal').classList.remove('hidden');
        ...
        document.getElementById('close-modal')?.addEventListener('click', function() {
            document.getElementById('biography-modal').classList.add('hidden');
        });
        // Modifica biografia
        document.querySelectorAll('.edit-biography-btn').forEach(function(btn) {
            btn.addEventListener('click', function(e) {
                e.preventDefault();
                document.getElementById('biography-form').reset();
                document.getElementById('biography-id').value = btn.dataset.id;
                document.getElementById('title').value = btn.dataset.title;
                document.getElementById('type').value = btn.dataset.type;
                document.querySelector('trix-editor[input="content"]').editor.loadHTML(btn
                    .dataset.content || '');
                document.getElementById('biography-modal').classList.remove('hidden');
            });
        });
