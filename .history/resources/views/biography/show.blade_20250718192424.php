<x-guest-layout>
            ...
            <div class="max-w-4xl px-4 py-12 mx-auto sm:px-6 lg:px-8">
                ...
                <div class="mb-16">
                    ...
                    <div class="mb-8 text-center">
                        <div
                            class="mb-4 inline-flex h-16 w-16 items-center justify-center rounded-full bg-gradient-to-r from-[#D4A574] to-[#E6B885]">
                        ...
                        <h2 class="mb-2 text-3xl font-bold text-white">{{ __('biography.main_biography_title') }}</h2>
                        <div class="mx-auto h-1 w-24 rounded-full bg-gradient-to-r from-[#D4A574] to-[#E6B885]"></div>
                    </div>
                    ...
                    @if ($biography->getFirstMedia('featured_image'))
                        ...
                        <div class="mb-8">
                            <div
                                class="overflow-hidden rounded-2xl border-2 border-[#D4A574] bg-gray-800/50 shadow-2xl">
                                ...
                                @if ($featuredImage->getCustomProperty('caption'))
                                    <div class="bg-gradient-to-r from-[#1B365D]/80 to-[#2D5016]/80 p-6">
                                        <p class="font-medium text-center text-white">
                                            {{ $featuredImage->getCustomProperty('caption') }}</p>
                                    </div>
                                @endif
                            </div>
                        </div>
                    ...
                    <div
                        class="mb-8 rounded-2xl border-2 border-[#D4A574] bg-gradient-to-br from-gray-800/60 to-gray-900/60 p-8 shadow-2xl backdrop-blur-sm">
                        <div class="prose biography-content prose-base prose-invert max-w-none">
                            {!! $biography->content !!}
                        </div>
                    </div>
                        ...
                        <div class="mb-8">
                            <div class="flex items-center mb-6">
                                ...
                                <h3 class="text-2xl font-semibold text-white">
                                    {{ __('biography.main_gallery_title') }}
                                </h3>
                            </div>
                                ...
                                @foreach ($biography->getMedia('main_gallery') as $media)
                                    <div
                                        ...
                                        @if (Str::startsWith($media->mime_type, 'image/'))
                                            ...
                                            <div
                                                class="absolute inset-0 flex items-center justify-center transition-opacity bg-black bg-opacity-0 group-hover:bg-opacity-30">
                                                <a href="{{ $media->getUrl() }}" target="_blank"
                                                    class="transform rounded-full bg-gradient-to-r from-[#D4A574] to-[#E6B885] p-3 text-gray-900 opacity-0 transition-all duration-300 hover:scale-110 group-hover:opacity-100">
                                        ...
                                        @if ($media->getCustomProperty('caption'))
                                            <div class="p-4 bg-gradient-to-r from-gray-800 to-gray-700">
                                                <p class="text-sm text-center text-gray-300">
                                                    {{ $media->getCustomProperty('caption') }}</p>
                                            </div>
                                        @endif
                                    </div>
                                @endforeach
                ...
                @if ($biography->chapters && $biography->chapters->count() > 0)
                    <div class="relative mb-16">
                        <div class="absolute inset-0 flex items-center">
                            <div
                                class="border-gradient-to-r w-full border-t-2 from-transparent via-[#D4A574] to-transparent">
                            </div>
                        </div>
                        <div class="relative flex justify-center">
                            <div
                                class="rounded-full border-2 border-[#D4A574] bg-gradient-to-r from-[#1B365D] to-[#2D5016] px-6 py-3">
                                <div class="flex items-center space-x-3">
                                    <svg class="h-6 w-6 text-[#D4A574]" fill="none" stroke="currentColor"
                                        viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    ...
                                    <span
                                        class="text-xl font-bold text-white">{{ __('biography.chapters_section_title') }}</span>
                                    <svg class="h-6 w-6 text-[#D4A574]" fill="none" stroke="currentColor"
                                        viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10">
                                        ...
                                        <div class="mt-8">
                                            ...
                                            <div class="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
                                                @foreach ($chapter->getMedia('chapter_images') as $media)
                                                    <div
                                                        class="relative overflow-hidden transition-transform bg-gray-700 rounded-lg group hover:scale-105">
                                                        @if (Str::startsWith($media->mime_type, 'image/'))
                                                            ...
                                                            <div
                                                                class="absolute inset-0 flex items-center justify-center transition-opacity bg-black bg-opacity-0 group-hover:bg-opacity-30">
                                                                <a href="{{ $media->getUrl() }}" target="_blank"
                                                                    class="p-2 text-gray-900 transition-opacity bg-white rounded-full opacity-0 bg-opacity-90 group-hover:opacity-100">
    ...
    <style>
        /* Stili specifici per il contenuto della biografia */
        .biography-content {
            font-size: 1rem;
            line-height: 1.75;
            font-weight: normal;
