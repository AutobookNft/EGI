...
@section('title', 'Modifica Biografia')

@section('content')
<div class="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 py-12">
    <div class="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
        <div class="bg-gray-800/50 backdrop-blur-sm border border-gray-700 rounded-xl p-8 shadow-2xl">

            <!-- Header -->
            <div class="mb-8">
                <h1 class="text-3xl font-bold text-white mb-2">Modifica Biografia</h1>
                <p class="text-gray-300">Aggiorna la tua storia</p>
            </div>
            ...
            <!-- Form -->
            <form action="{{ route('biography.update', $biography) }}" method="POST" enctype="multipart/form-data" id="biography-form">
                @csrf
                @method('PUT')
                        ...
                        <div class="space-y-6">
                            ...
                            <div>
                                ...
                                <select id="type" name="type"
                                    ...
                                    <option value="single" {{ old('type', $biography->type) == 'single' ? 'selected' : '' }}>Biografia Singola</option>
                                    <option value="chapters" {{ old('type', $biography->type) == 'chapters' ? 'selected' : '' }}>Biografia a Capitoli</option>
                                </select>
                                @error('type')
                                    <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                <div class="trix-container">
                                    <input id="content-trix" name="content" type="hidden" value="{{ old('content', $biography->content) }}">
                            ...
                            <div>
                                ...
                                <p class="mt-1 text-xs text-gray-400">
                                    Descrizione breve che apparirà in anteprima (<span id="excerpt-count">{{ strlen(old('excerpt', $biography->excerpt) ?? '') }}</span>/500)
                                </p>
                                @error('excerpt')
                                    <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                                @enderror
                    ...
                    <div class="tab-content" id="media-tab">
                        <div class="space-y-6">
                            ...
                            @if($biography->media && $biography->media->count() > 0)
                                <div class="space-y-4">
                                    <h4 class="text-lg font-medium text-white">Immagini Esistenti</h4>
                                    <div class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3" id="existing-images-grid">
                                            ...
                                            <div class="relative group">
                                                <div class="relative overflow-hidden rounded-lg bg-gray-900 aspect-square">
                                                    ...
                                                    <div class="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-50 transition-opacity flex items-center justify-center">
                                                        <button type="button" onclick="removeImage({{ $media->id }})"
                            ...
                            <div>
                                ...
                                <p class="text-xs text-gray-400">
                                    Carica nuove immagini per la tua biografia. Formati supportati: JPG, PNG, WEBP (Max 2MB ciascuna)
                                </p>

                                <!-- Loading indicator -->
                                <div id="upload-loading" class="mt-2 hidden items-center space-x-2 rounded-lg bg-[#D4A574]/10 p-3 text-[#D4A574]">
                                ...
                                <div id="images-gallery" class="mt-6 space-y-4">
                                    <h4 class="text-lg font-medium text-white">Nuove Immagini</h4>
                                    <div id="images-grid" class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
                                        <!-- New images will be inserted here via JavaScript -->
                                    </div>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="space-y-4">
                                <div class="flex items-center justify-between">
                                    ...
                                    <label class="relative inline-flex cursor-pointer items-center">
                                        <input type="checkbox" name="is_public" id="is_public" class="peer sr-only" {{ old('is_public', $biography->is_public) ? 'checked' : '' }}>
                ...
                <!-- Hidden field for biography ID -->
                <input type="hidden" name="biography_id" value="{{ $biography->id }}">

                <!-- Action Buttons -->
                <div class="flex items-center justify-between border-t border-gray-700 pt-6 mt-8">
                    <div class="flex items-center space-x-4">
                        ...
                        <span class="text-sm text-gray-400">
                            Ultima modifica: {{ $biography->updated_at->format('d/m/Y H:i') }}
                        </span>
                    </div>
...
<script>
    // Set biography ID for JavaScript
