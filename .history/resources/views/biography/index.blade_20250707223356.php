            ...
            "mainEntity": {
                ...
                "itemListElement": [
                    @foreach($biographies as $index => $biography)
                    {
                        ...
                        "item": {
                            "@type": "Person",
                            "@id": "{{ route('biography.show', $biography->slug) }}",
            ...
            <div class="relative px-4 mx-auto max-w-7xl sm:px-6 lg:px-8">
                ...
                <div class="text-center">
                    ...
                    <p class="max-w-3xl mx-auto mt-6 text-xl leading-relaxed text-gray-300">
                        Scopri le biografie di creator, mecenati e visionari che stanno costruendo il futuro ecologico digitale
                    </p>
                    ...
                    @if($isAuthenticated)
                        ...
                        @endif
                    @else
                        <div class="flex flex-col items-center mt-8 space-y-4 sm:flex-row sm:justify-center sm:space-y-0 sm:space-x-4">
                ...
                <div class="flex flex-col items-center justify-between space-y-4 lg:flex-row lg:space-y-0">
                    ...
                    <div class="flex items-center space-x-4">
                        ...
                        <select id="sort-select" onchange="updateSort()"
                                class="px-4 py-2 text-sm text-white bg-gray-700 border border-gray-600 rounded-lg focus:ring-2 focus:ring-yellow-400 focus:border-transparent">
                            <option value="updated_at-desc" {{ $currentSort === 'updated_at' && $currentDirection === 'desc' ? 'selected' : '' }}>
                                Più recenti
                            </option>
                            <option value="updated_at-asc" {{ $currentSort === 'updated_at' && $currentDirection === 'asc' ? 'selected' : '' }}>
                                Meno recenti
                            </option>
                            <option value="title-asc" {{ $currentSort === 'title' && $currentDirection === 'asc' ? 'selected' : '' }}>
                                Titolo A-Z
                            </option>
                            <option value="title-desc" {{ $currentSort === 'title' && $currentDirection === 'desc' ? 'selected' : '' }}>
                                Titolo Z-A
                            </option>
                            <option value="created_at-desc" {{ $currentSort === 'created_at' && $currentDirection === 'desc' ? 'selected' : '' }}>
                                Prima creazione
                            </option>
                        </select>
                    </div>
                ...
                @if($biographies->count() > 0)
                    <div class="grid gap-8 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                        @foreach($biographies as $biography)
                            <article class="relative overflow-hidden transition-all duration-300 transform bg-gray-800 border border-gray-700 shadow-lg group rounded-xl hover:shadow-2xl hover:-translate-y-1 hover:border-yellow-400/50">

                                {{-- Biography Image --}}
                                <div class="relative h-48 overflow-hidden bg-gradient-to-br from-yellow-900/30 to-green-900/30">
                                    ...
                                    @else
                                        <div class="flex items-center justify-center w-full h-full">
                                            <div class="text-center">
                                                <svg class="w-16 h-16 mx-auto text-yellow-400/50" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path>
                                                </svg>
                                                <span class="mt-2 text-sm text-gray-400">{{ $biography->user->name }}</span>
                                            </div>
                                        </div>
                                    @endif
                                    ...
                                    @if(!$biography->is_public)
                                        <div class="absolute top-3 right-3">
                                            <span class="inline-flex items-center px-2 py-1 text-xs font-medium text-yellow-300 rounded-full bg-gray-900/80 backdrop-blur-sm">
                                ...
                                <div class="p-6">
                                    ...
                                    <div class="flex items-center mb-3">
                                        <div class="flex items-center justify-center w-8 h-8 rounded-full bg-gradient-to-br from-yellow-400 to-yellow-600">
                                    ...
                                    {{-- Biography Title --}}
                                    <h3 class="mb-2 text-lg font-bold text-white transition-colors line-clamp-2 group-hover:text-yellow-400">
                                        {{ $biography->title }}
                                    </h3>
                                    ...
                                    <a href="{{ route('biography.show', $biography->slug) }}"
                                       class="inline-flex items-center text-sm font-semibold text-yellow-400 transition-colors hover:text-yellow-300">
                                        Leggi biografia
                                        <svg class="w-4 h-4 ml-1 transition-transform group-hover:translate-x-1" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                    ...
                    <div class="py-20 text-center">
                        ...
                        <p class="max-w-md mx-auto mb-8 text-gray-400">
                            @if($accessLevel === 'public')
                                Non ci sono ancora biografie pubbliche da mostrare. Sii il primo a condividere la tua storia!
                            @else
                                Non hai ancora creato biografie. Inizia a raccontare la tua storia nel Nuovo Rinascimento.
                            @endif
                        </p>
                        @if($canCreateBiography)
