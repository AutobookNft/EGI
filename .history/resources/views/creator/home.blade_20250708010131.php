...
<x-creator-layout
    :title="$creator->name . ' - ' . __('creator.home.title_suffix')"
    :metaDescription="__('creator.home.meta_description', ['name' => $creator->name])">
...
{{-- Hero Section --}}
<section class="relative min-h-[60vh] bg-gradient-to-br from-gray-900 via-blu-algoritmo to-gray-900 overflow-hidden"
        role="banner"
        aria-label="{{ __('creator.home.hero_aria_label', ['name' => $creator->name]) }}">
    ...
    {{-- Hero Content --}}
    <div class="relative z-10 px-4 py-16 mx-auto max-w-7xl sm:px-6 lg:px-8 md:py-24">
        ...
        <div class="grid items-end grid-cols-1 gap-12 md:grid-cols-12 md:gap-8">
            ...
            <div class="flex flex-col items-center gap-6 md:col-span-8 sm:flex-row sm:items-end sm:gap-8">
                {{-- Avatar --}}
                {{-- Padmin: Aggiunto flex-shrink-0 per evitare che l'avatar si restringa su schermi stretti --}}
                <div class="relative flex-shrink-0 group">
                    <div class="w-32 h-32 overflow-hidden rounded-full shadow-2xl md:w-40 md:h-40 ring-4 ring-oro-fiorentino/30">
                ...
                <div class="flex flex-col text-center sm:text-left">
                    <h1 class="mb-1 text-3xl font-bold text-white md:text-5xl font-playfair">
                        {{ $creator->name }}
                    </h1>
                    @if($creator->tagline)
                        <p class="text-lg italic md:text-xl text-oro-fiorentino font-source-sans">
                            "{{ $creator->tagline }}"
                        </p>
                    @endif
                    ...
                    <div class="mt-3">
                        <p class="text-gray-400">
                            {{ $creator->name }} {{ $creator->last_name ? $creator->last_name : '' }} &middot; {{ __('creator.home.member_since', ['year' => $creator->created_at->format('Y')]) }}
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="flex flex-col items-center gap-6 md:col-span-4 md:items-end">
                ...
                <div class="flex gap-3">
                    @if(\App\Helpers\FegiAuth::check() && \App\Helpers\FegiAuth::id() !== $creator->id)
                        <button type="button"
                                class="px-6 py-2.5 bg-oro-fiorentino text-gray-900 rounded-full font-semibold hover:bg-oro-fiorentino/90 transition-all duration-300 shadow-lg hover:shadow-xl"
                        ...
                        <button type="button"
                                class="px-6 py-2.5 bg-verde-rinascita text-white rounded-full font-semibold hover:bg-verde-rinascita/90 transition-all duration-300 shadow-lg hover:shadow-xl"
                ...
                <div class="flex gap-8 text-center">
                    <div>
                        ...
                        <p class="text-sm tracking-wider text-gray-400 uppercase">{{ __('creator.home.stats.works') }}</p>
                    </div>
                    <div>
                        <p class="text-2xl font-bold text-white"
                        ...
                        <p class="text-sm tracking-wider text-gray-400 uppercase">{{ __('creator.home.stats.collections') }}</p>
                    </div>
                    <div>
                        <p class="text-2xl font-bold text-white"
                        ...
                        <p class="text-sm tracking-wider text-gray-400 uppercase">{{ __('creator.home.stats.patrons') }}</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="px-4 mx-auto max-w-7xl sm:px-6 lg:px-8">
            <div class="flex overflow-x-auto scrollbar-hide">
                ...
                <a href="{{ route('creator.portfolio', $creator->id) }}"
                   class="px-6 py-4 text-sm font-medium text-gray-300 border-b-2 border-transparent hover:text-white hover:border-gray-600 whitespace-nowrap">
                ...
                <a href="{{ route('biography.index', $creator->id) }}"
                   class="px-6 py-4 text-sm font-medium text-gray-300 border-b-2 border-transparent hover:text-white hover:border-gray-600 whitespace-nowrap">
    ...
    <main class="px-4 py-12 mx-auto max-w-7xl sm:px-6 lg:px-8">
        <div class="grid grid-cols-1 gap-8 lg:grid-cols-3">
            ...
            <aside class="space-y-8" aria-label="{{ __('creator.home.sidebar_aria_label') }}">
                {{-- Impact Score Widget --}}
                <div class="p-6 border bg-gradient-to-br from-verde-rinascita/20 to-verde-rinascita/10 rounded-xl border-verde-rinascita/30">
