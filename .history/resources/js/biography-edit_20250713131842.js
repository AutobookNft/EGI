...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function renderChapterForm() {
        chapterModalContent.innerHTML = `
            <h2 class="text-xl font-bold text-white mb-4">Aggiungi Capitolo</h2>
            <form id="chapter-create-form" class="space-y-4">
                <div>
                    <label class="block text-sm text-gray-300 mb-1">Titolo *</label>
                    <input type="text" name="title" required class="w-full rounded border border-gray-600 bg-gray-800 px-3 py-2 text-white">
                </div>
                <div>
                    <label class="block text-sm text-gray-300 mb-1">Contenuto *</label>
                    <textarea name="content" rows="4" required class="w-full rounded border border-gray-600 bg-gray-800 px-3 py-2 text-white"></textarea>
                </div>
    ...
    async function submitChapterForm(e) {
        ...
        try {
            ...
            const response = await fetch(`/api/biographies/${biographyId}/chapters`, {
                method: 'POST',
                headers: {
            ...
            if (!result.success) throw new Error(result.message || 'Errore creazione capitolo');
            // Aggiorna lista capitoli
            appendChapterToList(result.data);
            closeChapterModal();
        } catch (err) {
