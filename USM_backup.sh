...
backup_dest_c="${backup_base_dir}/USM_${timestamp}"  # Sottodirectory specifica per USM
LOG_FILE="/home/fabio/libraries/USMCompiled/USM_backup_log.txt"

# Directory sorgente (intero progetto UltraUploadSandbox)
source_dir="/home/fabio/sandbox/UltraUploadSandbox"
...
drive_d="/mnt/d/Il mio Drive/EGI_backup/USM_${timestamp}"
drive_e="/mnt/e/EGI_backup/USM_${timestamp}"
drive_h="/mnt/h/EGI_backup/USM_${timestamp}"

# Directory e file da escludere dal backup
exclude_dirs=(
    "--exclude=vendor/"
    ...
    # "--exclude=packages/ultra/errormanager/"
    "--exclude=packages/ultra/uploadmanager/"
...
echo "===== Inizio backup USM $(date) =====" >> "$LOG_FILE"
...
if rsync -avz "${exclude_dirs[@]}" "$source_dir/" "$backup_dest_c"; then
    echo "✓ Backup principale USM completato in: $backup_dest_c" >> "$LOG_FILE"
else
    echo "✗ ERRORE nel backup principale USM!" >> "$LOG_FILE"
    exit 1
fi
...
if check_drive "$drive_d" "D:"; then
    ...
    if rsync -av "$backup_dest_c/" "$drive_d"; then
        echo "✓ Copia USM su D: completata in $drive_d" >> "$LOG_FILE"
    else
        echo "✗ ERRORE nella copia USM su D:" >> "$LOG_FILE"
    fi
fi
...
if check_drive "$drive_e" "E:"; then
    ...
    if rsync -av "$backup_dest_c/" "$drive_e"; then
        echo "✓ Copia USM su E: completata in $drive_e" >> "$LOG_FILE"
    else
        echo "✗ ERRORE nella copia USM su E:" >> "$LOG_FILE"
    fi
fi
...
if check_drive "$drive_h" "H:"; then
    ...
    if rsync -av "$backup_dest_c/" "$drive_h"; then
        echo "✓ Copia USM su H: completata in $drive_h" >> "$LOG_FILE"
    else
        echo "✗ ERRORE nella copia USM su H:" >> "$LOG_FILE"
    fi
fi

# Fine log operazione
echo "===== Fine backup USM $(date) =====" >> "$LOG_FILE"
