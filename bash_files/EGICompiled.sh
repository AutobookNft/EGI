...
output_dir="/home/fabio/libraries/EGICompiled"

# Nomi dei file di output divisi per categoria
server_file="$output_dir/EGI_server_code.txt"
client_file="$output_dir/EGI_client_code.txt"
config_file="$output_dir/EGI_config_misc.txt"

log_file="$output_dir/EGI_compilation_log.txt"
timestamp=$(date +"%Y%m%d%H%M%S")
zip_file="$output_dir/EGI_compiled_$timestamp.zip"
hash_file="$output_dir/EGI_file_hashes.txt"
prev_hash_file="$output_dir/EGI_file_hashes_prev.txt"
shared_dir="/var/www/shared"
modified_files_file="$shared_dir/EGI_modified_files.txt"

# Directory target specifica per EGI
target_dir="/home/fabio/EGI"
...
generate_hashes() {
    echo "### Hash dei File Inclusi in EGI ###" > "$hash_file"
    find "$target_dir" \
        -type d \( -path "*/vendor" -o -path "*/node_modules" -o -path "*/.*" -o -path "*/storage/framework" -o -path "*/public" -o -path "*/resources/lang" \) -prune -o \
...
compare_hashes() {
    if [ -f "$prev_hash_file" ]; then
        echo "### File Modificati in EGI ###" > "$modified_files_file"
        ...
        echo "Nessun file hash precedente trovato per EGI." > "$modified_files_file"
    fi
}

# Inizia il log
echo "Inizio compilazione EGI: $(date)" > "$log_file"
server_count=0
client_count=0
config_count=0
excluded_count=0
...
echo -e "######## EGI - Codice Server - Generato il $(date) ########\n" > "$server_file"
echo -e "######## EGI - Codice Client - Generato il $(date) ########\n" > "$client_file"
echo -e "######## EGI - Configurazioni e Altro - Generato il $(date) ########\n" > "$config_file"
...
find "$target_dir" \
    -type d \( -path "*/vendor" -o -path "*/node_modules" -o -path "*/.*" -o -path "*/storage/framework" -o -path "*/public" -o -path "*/resources/lang" \) -prune -o \
    -type f \( -name "*.php" -o -name "*.ts" -o -name "*.js" -o -name "*.env" -o -name "*.config.js" -o -name "*.blade.php" -o -name "*.css" -o -name "*.ico" -o -name "*.txt" -o -name "*.json" -o -name "*.stub" \) -print | sort | while read -r file; do
        if [[ $file == *"vendor"* || $file == *"node_modules"* || $file == *"/.*"* || $file == *"storage/framework"* || $file == *"public"* || $file == *"resources/lang"* ]]; then
            excluded_files+=("$file")
            ((excluded_count++))
            continue
        fi
...
echo "Fine compilazione EGI: $(date)" >> "$log_file"
...
echo "Compilazione EGI completata!"
